...
subplot(1, 2, 1), imshow(u0, [0 255]), title('Given image initial contour(u0)');
phi = double(phi);
...
DiceSimilarityCoefficient = 2*nnz(Iseg&phi)/(nnz(Iseg) + nnz(phi));
Jaccard = DiceSimilarityCoefficient/(2-DiceSimilarityCoefficient);
fprintf(' DiceSimilarityCoefficient: %f\n',DiceSimilarityCoefficient);
fprintf(' Jaccard: %f\n',Jaccard);
