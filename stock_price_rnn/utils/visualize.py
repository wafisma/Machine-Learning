...
def plot_lstm_prediction(actual, prediction, title='Google Trading vs Prediction', y_label='Price USD', x_label='Trading Days', save=False):
    """
    Plots train, test and prediction
    :param actual: DataFrame containing actual data
    :param prediction: DataFrame containing predicted values
    ...
    plt.plot(actual, '#ff0505', label='Actual Close')
    plt.plot(prediction, '#0000FF', label='Predicted Close')
    ...
    if save:
        plt.savefig(title+"_"+datetime.datetime.now().strftime("%Y-%B-%d %H-%M-%S")+'.jpeg')
    else:
        plt.show()
    return
